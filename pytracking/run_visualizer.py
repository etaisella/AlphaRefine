...
def run_visualizer(tracker_name, tracker_param, dataset_name='otb', sequence='Basketball', refined_path=None, show_gui=1):
    ...
    if DISPLAY_CENTER:
        centerPoints = np.loadtxt('centerPoint.txt')
    ...
    for i, frame in enumerate(data_sequence.frames):
        ...
        if DISPLAY_CENTER and (i > 0):
            center_point = centerPoints[i - 1].astype(int)
            cv2.circle(img, (center_point[0], center_point[1]), 10, (255, 0, 0), 4)
